function [] = fit_robust_all_lats(fin,fout,latid,fit_type,start_time,stop_time);
...
for latid = 1:40
   latid
   g = load([fin int2str(latid) '.mat'],'rcal','rtime','count');
   gnucal = load([fin '_nucal' int2str(latid) '.mat'],'btobs_nucal');
   btobs(latid,:,:) = gnucal.btobs_nucal;
   rcal(latid,:,:) = g.rcal;
   rtime(latid,:,:) = g.rtime;
   count(latid,:,:) = g.count;
end

[nlat,nd,nf] = size(rcal);
...
for latid=1:40
   robs(latid,:,:) = squeeze(bt2rad(f,squeeze(btobs(latid,:,:))'))';
end

robs = squeeze(nanmean(robs));
rcal = squeeze(nanmean(rcal));
count = squeeze(nanmean(count));
rtime = squeeze(nanmean(rtime));
