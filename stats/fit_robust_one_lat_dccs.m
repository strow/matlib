...
ndi = find( dmtime(:,1) >= start_time & dmtime(:,1) <= stop_time);
nd = length(ndi);
...
all_b        = NaN(nf,10,4);
all_berr     = NaN(nf,10,4);
all_rms      = NaN(nf,10,4);
all_resid    = NaN(nd,nf,4);
all_bt_resid = NaN(nd,nf,4);
all_times    = NaN(nd,nf,4);
all_bcorr    = NaN(nf,10,10,4);

%btobs = real(rad2bt(f,robs'));
for i=1:4
btobs(:,:,i) = real(rad2bt(f,robs(:,:,i)'));
end% btcal = real(rad2bt(f,rcal'));
% bias = btobs-btcal;
% bias = bias';
...
%if fit_type == 'rclr'
   for i=ig
      k = remove_6sigma(robs(:,i,ifov));
      j = remove_6sigma(robs(k,i,ifov));
      ind(i).k = k(j);
   end
   %end
...
it = ind(i).k;
   ...
   switch fit_type
     ...
     case 'rcal'
       fity = squeeze(rcal(it,i,ifov));
%      case 'rclr'
%        fity = squeeze(rclr(it,i));
     case 'bias'
       fity = squeeze(bias(it,i,ifov));       
   end
...
switch fit_type
  case {'robs', 'rcal', 'rclr'}
    ...
    dbt(:,ifov)     = all_b(:,2,ifov)./(deriv*1E3);
    dbt_err(:,ifov) = all_berr(:,2,ifov)./(deriv*1E3);
  case 'bias'
    dbt     = all_b(:,2);
    dbt_err = all_berr(:,2);
end
...
%   case {'robs', 'rcal', 'rclr'}
%     for i=1:nd
%        all_bt_resid(i,:,ifov) = all_resid(i,:)./(deriv'*1E3);
%     end
%   case 'bias'
%     for i=1:nd
...
if ~smallsave
   switch fit_type
     case {'robs', 'rcal', 'rclr'}
       save([fout int2str(latid_out)],'dbt','dbt_err','all*', 'deriv');
