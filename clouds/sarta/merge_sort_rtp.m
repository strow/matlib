function [head, prof] = merge_sort_rtp(head1, prof1,  head2, prof2, index1, index2);
...
%    index1,index2 : tells you how to merge and sort them
% Output:
...
%% sanity checks of index1, index2
iaAll = length(prof1.rlat) + length(prof2.rlat);
if iaAll ~= (length(index1) + length(index2))
  error('merge_sort_rtp.m : sum of elements in prof1,prof2 is NOT same as sorting arrays index1,index2')
end

if length(intersect(index1,index2)) ~= 0
  error('index1,index2 contain common elements')
end

iaAll = 1 : iaAll;
woo = sort([index1 index2]);
if sum(iaAll-woo) ~= 0
  error('huh? index1,index2 combined are not same as 1:length(all prof elements)')
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% now that we have passed the tests, see how to resort the combined structure
%{
%% simple testing 
ind1 = [1 4 6 8]; ind2 = setdiff(1:10,ind1);
intersect(index_cloud_selected,index_other)
plot(diff(sort([index_cloud_selected index_other])))
combine = [ind1 ind2];
all = 1 : length(combine);
[C,IA,IB] = intersect(all,combine); %%% we need IB
%}
...
fieldsIN  = fieldnames(prof);
for ii = 1 : length(fieldsIN)
  str = ['blah = prof.' fieldsIN{ii} ';'];
  eval(str);
  [mm,nn] = size(blah);
  if mm == 1
    %% this is like eg p.stemp = 1 x N 
    blah = blah(IB);
  else
    %% this is like eg p.robs1 = L x N ---> L x M
    blah = blah(:,IB);
  end
  str = ['prof.' fieldsIN{ii} ' = blah;'];
  eval(str)
end