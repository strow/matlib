...
for ibox =1:nboxes
  ...
  for i = 1: length(idc)
    ...
    if ICT(ilev,ibox)>1e-10
      ...
      %    vol. 97, pp. 3831-3836.
      qi = ICT(ilev,ibox)/ TT(ilev,ibox) *P(ilev)*100/R *1e3;  %change ice water content from kg/kg to g/m^3  
    ...
    qw = WCT(ilev,ibox)/ TT(ilev,ibox) *P(ilev)*100/R *1e3;  %change liquid water content from kg/kg to g/m^3
...
title('Rad 1231 : (b) cloudy (k) clear sky')
pause(0.1)
