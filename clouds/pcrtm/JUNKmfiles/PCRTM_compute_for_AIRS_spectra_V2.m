function [rad_allsky rad_clrsky tmpjunk] = PCRTM_compute_for_AIRS_spectra_V2(nboxes,nlev, ncol, overlap, P, WCT, ICT, cc, TT, q, o3, Ps, Ts, sfctype,efreq,emis,zen_ang,co2, parname,ppath)
...
for ibox =1:nboxes
  ...
  for ilev =indU(end) + 1:length(Pres)
    ...
    newT(ilev) = interp1(log(P(:,ibox)), TT(:,ibox), log(Pp));
    newh2o(ilev) = exp(interp1(log(P(:,ibox)), log(q(:,ibox)), log(Pp)));         
    newozone(ilev) = exp(interp1(log(P(:,ibox)), log(o3(:,ibox)), log(Pp)));  
  end
  ...
  Z = zeros(nlev,1);         
  for ilev = length(P(:,ibox))-1:-1:1
    Ttmp = (TT(ilev,ibox) + TT(ilev+1,ibox))/2;
    scaleH = R*Ttmp/g/1000;
    dz = scaleH*log(P(ilev+1,ibox)/P(ilev,ibox));
    Z(ilev) = Z(ilev+1) + dz; 
  end
...
%%%%    0 = end cloud info
%whos cldpres cldphase cldde cldopt cld_qw cld_iw
%  tmpjunk.cldpres(ibox,:,:)  = cldpres;
%  tmpjunk.cldphase(ibox,:,:) = cldphase;
%  tmpjunk.cldde(ibox,:,:)    = cldde;
%  tmpjunk.cldopt(ibox,:,:)   = cldopt;
%  tmpjunk.cld_qw(ibox,:,:)   = cld_qw;
%  tmpjunk.cld_qi(ibox,:,:)   = cld_qi;

  if ibox == nboxes  
    endsign=1;       
  end   
  make_PCRTM2AIRS_input_V2(Ps(ibox), Ts(ibox),Pres, newT, newh2o, newozone,...
                           ucol_num(ibox),ucol_num_same(ibox,:),cldnum,cld_id, cldpres, cldopt, cldde, cldphase, cld_qw, cld_qi,...
