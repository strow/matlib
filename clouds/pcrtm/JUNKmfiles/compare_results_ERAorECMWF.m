...
gg = find(isfinite(tobs) & isfinite(tSARTA) & isfinite(tPCRTM));
fprintf(1,' >>> length(tobs) length(finite(all(t*))) = %6i %6i \n',length(tobs),length(gg))
fprintf(1,' >>> length(tobs) length(finite(all(t*))) = %6i %6i \n',length(tobs),length(gg))
fprintf(1,' >>> length(tobs) length(finite(all(t*))) = %6i %6i \n',length(tobs),length(gg))


dt = -100 : 1 : 100;
nsarta = hist(tobs(gg) - tSARTA(gg),dt);
npcrtm = hist(tobs(gg) - tPCRTM(gg),dt);
...
dt = 180 : 1 : 320;
nxobs   = hist(tobs(gg),dt);
nxsarta = hist(tSARTA(gg),dt);
nxpcrtm = hist(tPCRTM(gg),dt);
figure(2); plot(dt,nxsarta,dt,nxpcrtm,'r',dt,nxobs,'k','linewidth',2); grid 
  hl = legend('sarta','pcrtm','obs'); set(hl,'fontsize',10);
  title(['all points ' num2str(h.vchan(oo)) ' cm-1']);

[sum(nxobs) sum(nxsarta) sum(nxpcrtm) length(gg)]
