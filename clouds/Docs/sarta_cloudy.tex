...
%the effective particle size and height of the dust, and a further
...
%the \sasc code in an operational AIRS L2 retrieval.  The additional
...
%surface emissivity; in addition we would need cloud phase. An algorithm 
%similar to the current L2 retrieval could provide estimates of cloud
%heights and fractions, which can then be used to easily produce the
%required parameters; from the height(s) one infers phase.  
%%Work is needed to determine if a liquid cloud particle size 
%must be derived or if climatology can be used.
