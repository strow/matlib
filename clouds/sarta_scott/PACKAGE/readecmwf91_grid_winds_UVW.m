...
for il = 1:nlev 
   ...
   ii = find( iparam_W == 1 & ilevel == 1); 
   if (length(ii) ~= 1) 
      disp(['did not find W ' strlev ' in GRIB inventory']); 
      return 
   end 
   irec_W(il) = ii; 
 
end 
clear iparam iparam_CC iparam_U iparam_V iparam_W ilevel strlev
...
head.ncld = 4;               %%% 4 cloud/speed parameters being output
head.gcld = [1;2;3;4];       %%% CC, U, V, W
head.nchan = 0;
head.mwnchan = 0;
...
profX.wspd = zeros(nlev,nprof);
profX.omegaspd = zeros(nlev,nprof);
...
for il = 1:nlev
   ...
   junk = readgrib_rec(filename,irec_W(il));
   profX.omegaspd(il,:) = junk(i1D);

   %%% see pg 291 of Atmospheric Science by Hobbs : omega = -(rho)(g)(w)
   %%% where rho = kg/m3
   rho = 100 * profX.plevs(il,:);         %% convert mb to N/m2
   rho = rho/1.38e-23./profX.ptemp(il,:); %% convert to density 1/m3
   rho = rho * 1.26;                      %% convert to density kg/m3
   profX.wspd(il,:) = -profX.omegaspd(il,:)/(9.8)./rho;
