function [cfracOUT,cN,cOUT,cT,cB,cPeak] = combine_clouds4t3(cfracIN,...
                                             iN,iOUT,iT,iB,iPeak,plevs)

% this takes in parameters iN,iOUT,iT,iB,iPeak,plevs  and cfracIN
% checks it ; if there are iN = 4, then it combines the four clouds to make
%   three clouds for [cfracOUT,cN,cOUT,cT,cB,cPeak]
% where 
%   I/O
...
if iN == 4
  ...
  %% do weighted average for cloudfrac
  cfracOUT  = zeros(size(iOUT));
  for kk = 1 : 3
    power = 4;
    ifrac = cfracIN(cT(kk):cB(kk)); ifrac = ifrac.^power;
    iamnt = iOUT(cT(kk):cB(kk));
    cf    = sum(ifrac.*iamnt)/sum(iamnt);
    cfracOUT(cT(kk):cB(kk)) = cf.^(1/power);
    end
