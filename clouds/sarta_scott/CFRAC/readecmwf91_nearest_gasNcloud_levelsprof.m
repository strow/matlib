function [head,prof,profX]=readecmwf91_nearest_gasNcloud_levelsprof(fname, ...
                                                                     lat,lon);

% copied from readecmwf91_nearest_gasNcloud.m 
% but dumps in both gas AND cloud profile info into prof, so that "klayers"
% can be run! gas201 has water, while gas202 has ice
...
% [head, prof, profX]=readecmwf91_nearest_gasNcloud_levelsprof(fname,lat, lon);
...
%
% Input:
%    fname : (string) complete ECMWG GRIB file name
...
% Created: 26 Feb 2007  Sergio Machado
...
d = dir(fname);
if (length(d) ~= 1)
   disp(['did not find GRIB file=' fname]);
   return
end
...
[rec,param,level] = readgrib_inv(fname);
...
junk = readgrib_rec(fname,irec_SKT);
prof.stemp = junk(i1D);

junk = readgrib_rec(fname,irec_SP);
prof.spres = junk(i1D)/100; % convert Pa to hPa=mb

%if (irec_LSM > 0)
%   junk = readgrib_rec(fname,irec_LSM);
...
for il = 1:nlev
   junk = readgrib_rec(fname,irec_CC(il)); 
   profX.cc(il,:) = junk(i1D); 
 
   junk = readgrib_rec(fname,irec_CIWC(il)); 
   profX.ciwc(il,:) = junk(i1D); 
 
   junk = readgrib_rec(fname,irec_CLWC(il)); 
   profX.clwc(il,:) = junk(i1D); 

   junk = readgrib_rec(fname,irec_T(il));
   prof.ptemp(il,:) = junk(i1D);
   profX.ptemp(il,:) = junk(i1D); 

   junk = readgrib_rec(fname,irec_Q(il));
...
% in which case this code should do: gas_1 = gas_1 / (1 - gas_1).
   ...
   junk = readgrib_rec(fname,irec_O3(il));
...
junk = readgrib_rec(fname,irec_10U);
windu = junk(i1D);
junk = readgrib_rec(fname,irec_10V);
windv = junk(i1D);
...
junk = readgrib_rec(fname,irec_TCC);
prof.cfrac = junk(i1D);
profX.cfrac = junk(i1D); 

junk = readgrib_rec(fname,irec_CI);
prof.udef1 = junk(i1D);

%%% end of function %%%

tnow = toc; 
fprintf(1,' took %8.6f minutes to read in ECMWF gas+cld file \n',tnow/60); 

%%%%%%%%%%%%%%% now put in the "profX" cloud info into prof %%%%%%%%%%%%%%%%%
head.ngas = 4; 
head.glist = [  1  3 201 202]'; 
head.gunit = [21  21  21  21]'; 
head.gunit = [21  21  26  26]';  %%this is for "wet" mixing ratios 

prof.gas_201  = profX.clwc;    % water cloud 
prof.ctype    = ones(1,length(prof.stemp))*101;   %% ctype for water
prof.cpsize   = ones(1,length(prof.stemp))*15;    %% dme   for water

prof.gas_202    = profX.ciwc;    % ice cloud 
prof.udef(17,:) = ones(1,length(prof.stemp))*201;  %% ctype for ice
prof.udef(12,:) = ones(1,length(prof.stemp))*30;   %% dme   for ice
prof.udef(15,:) = prof.cfrac;                      %% cfrac2   for ice
prof.udef(16,:) = prof.cfrac;                      %% cfrac12  for ice,water
