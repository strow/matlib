function [cFrac,cT,cB,cOUT,cngwat,cTYPE,iFound] = combine_clouds(...
              iCC,iN,iOUT,iT,iB,iPeak,wCC,wN,wOUT,wT,wB,wPeak,plevs,rlevs)

% this takes in ICE  parameters iN,iOUT,iT,iB,iPeak
%          and WATER parameters wN,wOUT,wT,wB,wPeak
%          and cloud fractions iCC,wCC
% and combines it for [cFrac,cT,cB,cOUT,cngwat,cTYPE,iFound] 
% where 
%   input
%      xCC == cloud fractions associated with cloud of type X
...
if iN == 2 & wN == 0
  ...
  cPeak(1) = max(iCC(cT(1):cB(1)));
  cPeak(2) = max(iCC(cT(2):cB(2)));
  cFrac(1) = cngwat(1)*cPeak(1)/cngwat(1);
  cFrac(2) = cngwat(2)*cPeak(2)/cngwat(2);
  end
...
if iN == 0 & wN == 2
  ...
  cPeak(1)  = max(wCC(cT(1):cB(1)));
  cPeak(2)  = max(wCC(cT(2):cB(2)));
  cFrac(1) = cngwat(1)*cPeak(1)/cngwat(1);
  cFrac(2) = cngwat(2)*cPeak(2)/cngwat(2);
  end
