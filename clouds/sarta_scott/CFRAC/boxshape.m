function [cOUT,yOUT,wT,wB,peakN,N,maxN,minN] = boxshape(cIN,yIN);
...
%%            cIN   cloud function that will be boxed the same way as yIN     
...
%%           cOUT   vector containing box appro to second approx
...
if sumYmax == 1 
  ...
  jj = 1;
  peakN(1) = sum(yIN(wT(jj):wB(jj)))/(wB-wT+1);
  yOUT(wT:wB) = yOUT(wT:wB) + peakN(1);
  ppN(1)    = sum(cIN(wT(jj):wB(jj)))/(wB-wT+1);
  cOUT(wT:wB) = cOUT(wT:wB) + ppN(1);
  iFound = +1;
  end
...
if sumYmax == 2
  ...
  ppN(jj) = sum(cIN(wT(jj):wB(jj)))/(wB(jj)-wT(jj)+1);
  cOUT(wT(jj):wB(jj)) = cOUT(wT(jj):wB(jj)) + ppN(jj);
