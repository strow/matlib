function [tai] = utc2tai(year, month, day, dhour);

%
% function [tai] = utc2tai(year, month, day, dhour);
%
% Convert UTC time to approximate seconds since 1 Jan 1993 (AIRS tai time)
%
% Input:
...
%
% Output:
%    tai : (1 x n) AIRS TAI time (seconds since 00:00 1 Jan 1993)
%
...
n=length(year);
iyear=round(year);
if ( max(abs(year - iyear)) > 0.01 | min(iyear) < 1993 | max(iyear) > 2099 )
   error('year must be an integer 1993 to 2099')
end
if (irow == n)
   iyear=iyear';
end
...
ii=iyear - 1993 + 1;  % index for taiyear
