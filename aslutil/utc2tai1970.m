function [tai] = utc2tai1970(year, month, day, dhour);

%
% function [tai] = utc2tai1970(year, month, day, dhour);
%
% Convert UTC time to approximate seconds since 1 Jan 1970
%
% Input:
...
%
% Output:
%    tai : (1 x n) TAI time (seconds since 00:00 1 Jan 1970)
%

% Created: 23 October 2002 Scott Hannon
% Update: 26 Mar 2009, S.Hannon tai1970 variant created
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
n=length(year);
iyear=round(year);
if ( max(abs(year - iyear)) > 0.01 | min(iyear) < 1970 | max(iyear) > 2099 )
   error('year must be an integer 1970 to 2099')
end
if (irow == n)
   iyear=iyear'; %'
end
...
ii=iyear - 1970 + 1;  % index for taiyear
